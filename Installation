...
If you do not want to install git, you can instead download [master.zip](https://github.com/ageron/handson-ml2/archive/master.zip), unzip it, rename the resulting directory to `LCS_OECS` and move it to your development directory.
...
## Create the `LCS` Environment
Next, make sure you're in the `Jupyter_LCS` directory and run the following command. It will create a new `conda` environment containing every library you will need to run all the notebooks (by default, the environment will be named `LCS`, but you can choose another name using the `-n` option):
...
## Start Jupyter
It's almost done! You just need to register the `LCS` conda environment to Jupyter. The notebooks in this project will default to the environment named `python3`, so it's best to register this environment using the name `python3` (if you prefer to use another name, you will have to select it in the "Kernel > Change kernel..." menu in Jupyter every time you open a notebook):
...
Congrats! You are now ready to learn how to extract LCS/OECS from velocity data!
...
For this, open a terminal, and run:
    ...
    $ cd Name-of-Directory # or whatever development directory you chose earlier
    $ cd LCS_OECS # go to this project's directory
    $ git pull
...
Then we'll delete this project's `LCS` environment:

    $ conda activate base
    $ conda env remove -n LCS
